...
TEST( findSymbolInString ) {
    // Symbol present
    {
        ASSERT_EQ( "%zd", findSymbolInString( "hello", 'e' ), ( size_t )1 );
        ASSERT_EQ( "%zd", findSymbolInString( "abcdef", 'd' ), ( size_t )3 );
    }

    // Symbol at the beginning
    ASSERT_EQ( "%zd", findSymbolInString( "world", 'w' ), ( size_t )0 );

    // Symbol at the end
    ASSERT_EQ( "%zd", findSymbolInString( "test", 't' ), ( size_t )0 );

    // Symbol not present
    ASSERT_EQ( "%zd", findSymbolInString( "abc", 'z' ), ( size_t )( -1 ) );

    // Empty string
    ASSERT_EQ( "%zd", findSymbolInString( "", 'x' ), ( size_t )( -1 ) );

    // NULL string
    ASSERT_EQ( "%zd", findSymbolInString( NULL, 'a' ), ( size_t )( -1 ) );
}

TEST( findLastSymbolInString ) {
    // Last occurrence in middle
    ASSERT_EQ( "%zd", findLastSymbolInString( "hello", 'l' ), ( size_t )3 );

    // Symbol at the end
    ASSERT_EQ( "%zd", findLastSymbolInString( "abcdef", 'f' ), ( size_t )5 );

    // Symbol at the beginning
    ASSERT_EQ( "%zd", findLastSymbolInString( "test", 't' ), ( size_t )3 );

    // Multiple occurrences, last one should be returned
    ASSERT_EQ( "%zd", findLastSymbolInString( "banana", 'a' ), ( size_t )5 );

    // Symbol not present
    ASSERT_EQ( "%zd", findLastSymbolInString( "xyz", 'a' ), ( size_t )( -1 ) );

    // Empty string
    ASSERT_EQ( "%zd", findLastSymbolInString( "", 'x' ), ( size_t )( -1 ) );

    // NULL string
    ASSERT_EQ( "%zd", findLastSymbolInString( NULL, 'a' ), ( size_t )( -1 ) );
}
