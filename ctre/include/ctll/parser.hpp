...
struct parser { // in c++20
    ...
    struct results {
        ...
        constexpr auto operator+( placeholder ) const noexcept {
            if constexpr ( Decision == decision::undecided ) {
                ...
                // :)
                return parser< Grammar, _input, ActionSelector,
                               IgnoreUnknownActions >::
                    template decide< Pos, Stack, Subject >( {}, {} );
            } else {
                // if there is decision already => just push it to the end of
                // fold expression
                return *this;
            }
        }
    ...
    static constexpr auto decide( Stack previous_stack,
        ...
        [[maybe_unused]] auto stack =
            decltype( ctll::pop_front( previous_stack ) )();

        // in case top_symbol is action type (apply it on previous subject and
        // get new one)
        if constexpr ( std::is_base_of_v< ctll::action,
                                          decltype( top_symbol ) > ) {
            ...
            if constexpr ( std::is_same_v< ctll::reject,
                                           decltype( subject ) > ) {
                return typename parser< Grammar, _input, ActionSelector,
                                        IgnoreUnknownActions >::
                    template results< Pos, Stack, Subject, decision::reject >();
