...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< start_mark, Tail... > )  {
    return evaluate( begin, current, last, f,
                     captures.set_start_mark( current ),
                     ctll::list< Tail... >() );
}
...
constexpr CTRE_FORCE_INLINE bool match_string(
    ...
    [[maybe_unused]] const flags& f ) {
    return ( ( current != last &&
               character< String >::match_char( *current++, f ) ) &&
             ... && true );
}
...
constexpr CTRE_FORCE_INLINE R evaluate(
    ...
    const flags& f,
    R captures,
    ctll::list< select< HeadOptions, TailOptions... >, Tail... > )  {
...
constexpr CTRE_FORCE_INLINE R evaluate(
    ...
    const flags& f,
    R captures,
    ctll::list< sequence< HeadContent, TailContent... >, Tail... > )  {
...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< boundary< CharacterLike >, Tail... > )  {
...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< not_boundary< CharacterLike >, Tail... > )  {
    // reason why I need bidirectional iterators or some clever hack
    bool before = false;
    bool after = false;
...
evaluate( const BeginIterator begin,
    ...
    if constexpr ( B != 0 && A > B ) {
        ...
        while ( less_than_or_infinite< B >( i ) ) {
            ...
            auto outer_result =
                evaluate( begin, inner_result.get_end_position(), last,
                          consumed_something( f ), inner_result.unmatch(),
                          ctll::list< Tail... >() );
        ...
        // rest of regex
        return evaluate( begin, current, last, consumed_something( f ),
                         captures, ctll::list< Tail... >() );
    }
}
...
#ifdef CTRE_MSVC_GREEDY_WORKAROUND
        R result;
        evaluate_recursive( result, i, begin, current, last, f, captures,
                            stack );
        return result;
#else
        return evaluate_recursive( i, begin, current, last, f, captures,
                                   stack );
#endif
    }
}
...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< capture_with_name< Id, Name, Content... >,
                      Tail... > )  {
    return evaluate(
        begin, current, last, f,
        captures.template start_capture< Id >( current ),
...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< back_reference_with_name< Id >, Tail... > )  {
    if ( const auto ref = captures.template get< Id >() ) {
        if ( auto result = match_against_range( current, last, ref.begin(),
                                                ref.end(), f );
             result.match ) {
...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< lookbehind_positive< Content... >, Tail... > )  {
    static_assert(
...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< lookbehind_negative< Content... >, Tail... > )  {
    static_assert(
        is_at_least_bidirectional(
            typename std::iterator_traits< Iterator >::iterator_category{} ),
...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< atomic_group< Content... >, Tail... > )  {
    return evaluate(
        begin, current, last, f, captures,
...
evaluate( const BeginIterator begin,
          ...
          const flags& f,
          R captures,
          ctll::list< mode_switch< Mode >, Tail... > )  {
